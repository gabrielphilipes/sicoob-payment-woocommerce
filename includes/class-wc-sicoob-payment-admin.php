...
class WC_Sicoob_Payment_Admin {
	...
	public function add_admin_menu() {
		add_submenu_page(
			'woocommerce',
			esc_html__( 'Sicoob Payment', 'sicoob-payment' ),
			esc_html__( 'Sicoob Payment', 'sicoob-payment' ),
	...
	public function enqueue_admin_scripts( $hook ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only data from URL parameters.
		if ( strpos( $hook, 'wc-settings' ) !== false && isset( $_GET['section'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only data from URL parameters.
			$section = sanitize_text_field( wp_unslash( $_GET['section'] ) );

			if ( in_array( $section, array( 'sicoob_pix', 'sicoob_boleto' ), true ) ) {
				wp_enqueue_style(
					'sicoob-payment-admin',
					SICOOB_PAYMENT_PLUGIN_URL . 'assets/css/admin.css',
					array(),
	...
	public function save_logs_config() {
		// Verificar nonce.
		if ( ! isset( $_POST['sicoob_logs_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['sicoob_logs_nonce'] ) ), 'sicoob_save_logs_config' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}

		// Verificar permissões.
		if ( ! current_user_can( 'manage_woocommerce' ) ) { // phpcs:ignore WordPress.WP.Capabilities.Unknown -- WooCommerce capability.
			wp_die( esc_html__( 'Você não tem permissão para realizar esta ação.', 'sicoob-payment' ) );
		}
	...
	public function save_auth_config() {
		// Verificar nonce.
		if ( ! isset( $_POST['sicoob_auth_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['sicoob_auth_nonce'] ) ), 'sicoob_save_auth_config' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
		...
		if ( ! empty( $_POST['client_id'] ) ) {
			$config['client_id'] = sanitize_text_field( wp_unslash( $_POST['client_id'] ) );
		} elseif ( ! empty( $_POST['existing_client_id'] ) ) {
			// Se o campo principal está vazio mas há um valor existente, manter o existente.
			$config['client_id'] = sanitize_text_field( wp_unslash( $_POST['existing_client_id'] ) );
		}
			...
			if ( $upload_result['success'] ) {
				...
				$redirect_url = add_query_arg(
					array(
						...
						'error_message' => rawurlencode( $upload_result['message'] ),
					),
					admin_url( 'admin.php' )
				);
				wp_safe_redirect( $redirect_url );
	...
	private function handle_certificate_upload() {
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verification is handled in the calling method.
		if ( ! isset( $_FILES['certificate_file'] ) ) {
			return array(
				'success' => false,
				'message' => esc_html__( 'Nenhum arquivo foi enviado.', 'sicoob-payment' ),
			);
		}
		$file = $_FILES['certificate_file']; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

		// Verificar se há erro no upload.
		if ( UPLOAD_ERR_OK !== $file['error'] ) {
			return array(
				'success' => false,
				'message' => esc_html__( 'Erro no upload do arquivo.', 'sicoob-payment' ),
			);
		}
		...
		if ( ! in_array( $file_extension, $allowed_extensions, true ) ) {
			return array(
				'success' => false,
				'message' => esc_html__( 'Tipo de arquivo não permitido. Use apenas arquivos .PEM, .CRT ou .KEY.', 'sicoob-payment' ),
			);
		}
		...
		if ( $file['size'] > 1024 * 1024 ) {
			return array(
				'success' => false,
				'message' => esc_html__( 'Arquivo muito grande. Tamanho máximo permitido: 1MB.', 'sicoob-payment' ),
			);
		}

		// Validar conteúdo do certificado.
		$certificate_content = file_get_contents( $file['tmp_name'] ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Local file validation.
		if ( ! $this->validate_certificate_content( $certificate_content ) ) {
			return array(
				'success' => false,
				'message' => esc_html__( 'Arquivo de certificado inválido. Verifique se é um certificado válido do Sicoob.', 'sicoob-payment' ),
			);
		}
		...
		if ( move_uploaded_file( $file['tmp_name'], $file_path ) ) {
			...
			chmod( $file_path, 0600 ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_chmod -- Certificate file permissions.

			return array(
				'success'   => true,
				'file_path' => $file_path,
				'message'   => esc_html__( 'Certificado carregado com sucesso.', 'sicoob-payment' ),
			);
		} else {
			return array(
				'success' => false,
				'message' => esc_html__( 'Erro ao salvar o arquivo. Verifique as permissões do diretório.', 'sicoob-payment' ),
			);
		}
	}
	...
	public function remove_certificate() {
		// Verificar nonce.
		if ( ! isset( $_POST['sicoob_remove_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['sicoob_remove_nonce'] ) ), 'sicoob_remove_certificate' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
		...
		$redirect_url = add_query_arg(
			array(
				...
				'error_message' => $result['success'] ? '' : rawurlencode( $result['message'] ),
			),
			admin_url( 'admin.php' )
		);
	...
	public function ajax_remove_certificate() {
		// Verificar nonce.
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'sicoob_remove_certificate' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
	...
	private function process_certificate_removal() {
		...
		if ( empty( $config['certificate_path'] ) ) {
			return array(
				'success' => false,
				'message' => esc_html__( 'Nenhum certificado encontrado para remover.', 'sicoob-payment' ),
			);
		}
		...
		if ( file_exists( $certificate_path ) ) {
			if ( ! wp_delete_file( $certificate_path ) ) {
				return array(
					'success' => false,
					'message' => esc_html__( 'Erro ao remover o arquivo de certificado.', 'sicoob-payment' ),
				);
			}
		}
		...
		return array(
			'success' => true,
			'message' => esc_html__( 'Certificado removido com sucesso.', 'sicoob-payment' ),
		);
	}
	...
	public function ajax_test_api() {
		// Verificar nonce.
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'sicoob_test_api' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
		...
		$scope = ( 'boleto' === $scope_type ) ? WC_Sicoob_Payment_API::BOLETO_SCOPE : WC_Sicoob_Payment_API::PIX_SCOPE;
	...
	public function ajax_test_pix_generation() {
		// Verificar nonce.
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) ), 'sicoob_test_pix_generation' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
		...
		if ( empty( $pix_settings['pix_key'] ) || empty( $pix_settings['pix_description'] ) ) {
			wp_send_json_error(
				array(
					'message'  => esc_html__( 'Configurações do PIX não estão completas. Configure a chave PIX e descrição nas configurações do gateway.', 'sicoob-payment' ),
	...
	public function ajax_test_boleto_generation() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) ), 'sicoob_test_boleto_generation' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
		...
		WC_Sicoob_Payment::log_message(
			sprintf(
				...
				esc_html__( 'Iniciando teste de geração de boleto - Pedido: %1$s, Cliente: %2$s', 'sicoob-payment' ),
				$test_data['order_id'],
				$test_data['nome']
			),
			'info'
		...
		if ( $result['success'] ) {
			WC_Sicoob_Payment::log_message(
				sprintf(
					/* translators: %s: Our number */
					esc_html__( 'Teste de boleto bem-sucedido - Nosso Número: %s', 'sicoob-payment' ),
			...
			WC_Sicoob_Payment::log_message(
				sprintf(
					...
					esc_html__( 'Teste de boleto falhou: %s', 'sicoob-payment' ),
					$result['message']
				),
				'error'
			);
	...
	private function validate_boleto_settings( $settings ) {
		...
		if ( empty( $settings['account_number'] ) ) {
			$missing_fields[] = esc_html__( 'Número da Conta Corrente', 'sicoob-payment' );
		}

		if ( empty( $settings['contract_number'] ) ) {
			$missing_fields[] = esc_html__( 'Número do Contrato', 'sicoob-payment' );
		}
		...
		if ( ! empty( $missing_fields ) ) {
			return array(
				'valid'          => false,
				'message'        => sprintf(
					/* translators: %s: Missing fields list */
					esc_html__( 'Configurações do Boleto incompletas. Campos obrigatórios: %s', 'sicoob-payment' ),
	...
	public function ajax_test_boleto_email() {
		// Verificar nonce de segurança.
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) ), 'sicoob_test_boleto_email' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
		...
		if ( empty( $test_email ) || ! is_email( $test_email ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'E-mail de teste inválido. Forneça um e-mail válido.', 'sicoob-payment' ),
				)
			);
		}
		...
		if ( ! isset( $emails['WC_Sicoob_Boleto_Email'] ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Classe de e-mail do boleto não está registrada. Verifique se o plugin está ativo.', 'sicoob-payment' ),
				)
			);
		}
		...
		WC_Sicoob_Payment::log_message(
			sprintf(
				/* translators: %1$s: Email address, %2$s: Order ID */
				esc_html__( 'Iniciando teste de e-mail de boleto - E-mail: %1$s, Pedido: %2$s', 'sicoob-payment' ),
				$test_email,
				$test_order->get_id()
			),
			'info'
		...
		try {
			...
			WC_Sicoob_Payment::log_message(
				sprintf(
					...
					esc_html__( 'Teste de e-mail de boleto bem-sucedido - E-mail: %s', 'sicoob-payment' ),
					$test_email
				),
				'info'
			);
			...
			$response_data = array(
				...
				'message'      => sprintf(
					/* translators: %s: Email address */
					esc_html__( 'E-mail de teste enviado com sucesso para %s!', 'sicoob-payment' ),
					$test_email
				),
				'test_summary' => array(
					'test_type'     => 'boleto_email',
			...
			WC_Sicoob_Payment::log_message(
				sprintf(
					/* translators: %s: Error message */
					esc_html__( 'Teste de e-mail de boleto falhou: %s', 'sicoob-payment' ),
					$e->getMessage()
				),
				'error'
			);
			...
			wp_send_json_error(
				array(
					...
					'message'       => sprintf(
						/* translators: %s: Error message */
						esc_html__( 'Erro ao enviar e-mail de teste: %s', 'sicoob-payment' ),
						$e->getMessage()
					),
	...
	private function generate_test_boleto_data() {
		return array(
			...
			'valor'           => 0.01,
			'data_vencimento' => gmdate( 'Y-m-d', strtotime( '+3 days' ) ),
			'data_emissao'    => gmdate( 'Y-m-d' ),
	...
	public function ajax_register_webhook() {
		// Verificar nonce.
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) ), 'sicoob_register_webhook' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
		...
		if ( empty( $pix_key ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Chave PIX não configurada. Configure a chave PIX nas configurações do gateway.', 'sicoob-payment' ),
				)
			);
		}
		...
		WC_Sicoob_Payment::log_message(
			sprintf(
				...
				esc_html__( 'Iniciando registro de webhook PIX - Chave: %1$s, URL: %2$s', 'sicoob-payment' ),
				$pix_key,
				$webhook_url
			),
			'info'
	...
	public function ajax_unregister_webhook() {
		// Verificar nonce.
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) ), 'sicoob_unregister_webhook' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
		...
		WC_Sicoob_Payment::log_message(
			sprintf(
				...
				esc_html__( 'Iniciando remoção de webhook PIX - Chave: %s', 'sicoob-payment' ),
				$pix_key
			),
			'info'
		);
	...
	public function ajax_check_webhook_status() {
		// Verificar nonce.
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) ), 'sicoob_check_webhook_status' ) ) {
			wp_die( esc_html__( 'Ação não autorizada.', 'sicoob-payment' ) );
		}
		...
		WC_Sicoob_Payment::log_message(
			sprintf(
				/* translators: %s: PIX key */
				esc_html__( 'Consultando status do webhook PIX - Chave: %s', 'sicoob-payment' ),
				$pix_key
			),
			'info'
		);
	...
	public function admin_page() {
		...
		<div class="wrap">
			...
			<?php if ( 'logs_saved' === $message ) : ?>
				<div class="sicoob-notice success">
					<span class="dashicons dashicons-yes-alt"></span>
					<?php esc_html_e( 'Configurações de logs salvas com sucesso!', 'sicoob-payment' ); ?>
				</div>
			<?php endif; ?>

			<?php if ( 'auth_saved' === $message ) : ?>
				<div class="sicoob-notice success">
					<span class="dashicons dashicons-yes-alt"></span>
					<?php esc_html_e( 'Configurações de autenticação salvas com sucesso!', 'sicoob-payment' ); ?>
				</div>
			<?php endif; ?>

			<?php if ( 'certificate_removed' === $message ) : ?>
				<div class="sicoob-notice success">
					<span class="dashicons dashicons-yes-alt"></span>
					<?php esc_html_e( 'Certificado removido com sucesso!', 'sicoob-payment' ); ?>
				</div>
			<?php endif; ?>

			<?php if ( 'certificate_upload_failed' === $error ) : ?>
				<div class="sicoob-notice error">
					<span class="dashicons dashicons-warning"></span>
					<?php
					// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only data from URL parameters.
					$error_message = isset( $_GET['error_message'] ) ? sanitize_text_field( wp_unslash( $_GET['error_message'] ) ) : esc_html__( 'Erro no upload do certificado.', 'sicoob-payment' );
					echo esc_html( $error_message );
					?>
				</div>
			<?php endif; ?>

			<?php if ( 'certificate_remove_failed' === $message ) : ?>
				<div class="sicoob-notice error">
					<span class="dashicons dashicons-warning"></span>
					<?php
					// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only data from URL parameters.
					$error_message = isset( $_GET['error_message'] ) ? sanitize_text_field( wp_unslash( $_GET['error_message'] ) ) : esc_html__( 'Erro ao remover o certificado.', 'sicoob-payment' );
					echo esc_html( $error_message );
					?>
				</div>
			<?php endif; ?>
			...
			<div class="sicoob-payment-admin">
				<p><?php esc_html_e( 'Bem-vindo à área administrativa do Sicoob Payment.', 'sicoob-payment' ); ?></p>
				<p><?php esc_html_e( 'Aqui você poderá configurar e gerenciar os pagamentos via PIX e Boleto do Sicoob.', 'sicoob-payment' ); ?></p>
				...
				<div class="sicoob-config-block">
					<div class="sicoob-config-block-header">
						<h2>
							...
							<?php esc_html_e( 'Dados de Autenticação', 'sicoob-payment' ); ?>
						</h2>
					</div>
					...
					<div class="sicoob-config-block-content">
						...
						<div class="sicoob-security-alert">
							...
							<div class="sicoob-security-alert-content">
								<div class="sicoob-security-alert-title">
									<?php esc_html_e( 'Dados Sensíveis', 'sicoob-payment' ); ?>
								</div>
								<div class="sicoob-security-alert-text">
									<?php esc_html_e( 'As informações abaixo são sensíveis e serão utilizadas para autenticação com a API do Sicoob. Mantenha-as seguras e não as compartilhe.', 'sicoob-payment' ); ?>
								</div>
							</div>
						</div>
						...
						<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" enctype="multipart/form-data" class="sicoob-config-form">
							...
							<div class="sicoob-config-field">
								<label for="client_id">
									<?php esc_html_e( 'ID do Cliente', 'sicoob-payment' ); ?>
								</label>
								
								<?php if ( ! empty( $auth_config['client_id'] ) ) : ?>
									<div class="sicoob-sensitive-status configured">
										<span class="dashicons dashicons-yes-alt"></span>
										<?php esc_html_e( 'ID do Cliente configurado', 'sicoob-payment' ); ?>
									</div>
								<?php else : ?>
									<div class="sicoob-sensitive-status not-configured">
										<span class="dashicons dashicons-warning"></span>
										<?php esc_html_e( 'ID do Cliente não configurado', 'sicoob-payment' ); ?>
									</div>
								<?php endif; ?>
								...
								<div class="sicoob-input-with-button">
									<input 
										...
										name="client_id" 
										value=""
										placeholder="<?php echo ! empty( $auth_config['client_id'] ) ? '••••••••••' : esc_html__( 'Digite o ID do Cliente', 'sicoob-payment' ); ?>"
									...
									<?php if ( ! empty( $auth_config['client_id'] ) ) : ?>
										<button type="button" class="sicoob-change-data-btn">
											...
											<?php esc_html_e( 'Alterar', 'sicoob-payment' ); ?>
										</button>
									<?php endif; ?>
								</div>
								
								<div class="sicoob-config-field-description">
									<?php esc_html_e( 'ID do Cliente disponibilizado no painel de desenvolvedor do Sicoob.', 'sicoob-payment' ); ?>
								</div>
							</div>
							...
							<div class="sicoob-config-field">
								<label for="certificate_file">
									<?php esc_html_e( 'Certificado Digital (.PEM)', 'sicoob-payment' ); ?>
								</label>
								
								<div class="sicoob-config-field-description">
									<?php esc_html_e( 'Certificado digital no formato .PEM obtido no painel de desenvolvedor do Sicoob. Será utilizado para autenticação nas requisições cURL.', 'sicoob-payment' ); ?>
								</div>

								<?php if ( ! empty( $auth_config['certificate_path'] ) && file_exists( $auth_config['certificate_path'] ) ) : ?>
									<div class="sicoob-sensitive-status configured">
										<span class="dashicons dashicons-yes-alt"></span>
										<?php esc_html_e( 'Certificado configurado', 'sicoob-payment' ); ?>
										<div class="sicoob-certificate-info">
											<small><?php esc_html_e( 'Arquivo:', 'sicoob-payment' ); ?> <?php echo esc_html( basename( $auth_config['certificate_path'] ) ); ?></small>
										</div>
										<div class="sicoob-certificate-actions">
											<button type="button" class="sicoob-remove-certificate-btn" data-certificate-path="<?php echo esc_attr( $auth_config['certificate_path'] ); ?>">
												...
												<?php esc_html_e( 'Remover Certificado', 'sicoob-payment' ); ?>
											</button>
										</div>
									</div>
								<?php else : ?>
									<div class="sicoob-sensitive-status not-configured">
										<span class="dashicons dashicons-warning"></span>
										<?php esc_html_e( 'Certificado não configurado', 'sicoob-payment' ); ?>
									</div>
								<?php endif; ?>
								...
								<div class="sicoob-file-upload-wrapper">
									...
									<div class="sicoob-file-upload-info">
										<div class="sicoob-file-requirements">
											<strong><?php esc_html_e( 'Requisitos do arquivo:', 'sicoob-payment' ); ?></strong>
											<ul>
												<li><?php esc_html_e( 'Formato: .PEM, .CRT ou .KEY', 'sicoob-payment' ); ?></li>
												<li><?php esc_html_e( 'Tamanho máximo: 1MB', 'sicoob-payment' ); ?></li>
												<li><?php esc_html_e( 'Certificado válido do Sicoob', 'sicoob-payment' ); ?></li>
											</ul>
										</div>
							...
							<div class="sicoob-config-actions">
								<button type="submit" class="sicoob-btn sicoob-btn-primary">
									...
									<?php esc_html_e( 'Salvar Configurações', 'sicoob-payment' ); ?>
								</button>
							</div>
						</form>
					</div>
				...
				<div class="sicoob-config-block">
					<div class="sicoob-config-block-header">
						<h2>
							...
							<?php esc_html_e( 'Configurações de Logs', 'sicoob-payment' ); ?>
						</h2>
					</div>
						...
						<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="sicoob-config-form">
							...
							<div class="sicoob-config-field">
								<div class="sicoob-checkbox-wrapper">
									...
									<label for="enable_logs">
										<?php esc_html_e( 'Habilitar registro de logs em arquivos', 'sicoob-payment' ); ?>
										<span class="sicoob-checkbox-status <?php echo 'yes' === $logs_enabled ? 'enabled' : 'disabled'; ?>">
											<span class="dashicons <?php echo 'yes' === $logs_enabled ? 'dashicons-yes-alt' : 'dashicons-dismiss'; ?>"></span>
											<?php echo 'yes' === $logs_enabled ? esc_html__( 'Ativado', 'sicoob-payment' ) : esc_html__( 'Desativado', 'sicoob-payment' ); ?>
										</span>
									</label>
								</div>
								
								<div class="sicoob-config-field-description">
									<?php esc_html_e( 'Quando habilitado, o plugin registrará logs detalhados das transações e operações em arquivos. Isso é útil para depuração e monitoramento, mas pode gerar muitos arquivos de log.', 'sicoob-payment' ); ?>
								</div>
							</div>
							...
							<div class="sicoob-config-actions">
								...
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-status&tab=logs' ) ); ?>" class="sicoob-btn sicoob-btn-secondary">
									...
									<?php esc_html_e( 'Ver Logs do WooCommerce', 'sicoob-payment' ); ?>
								</a>
							</div>
						</form>
					</div>
				...
				<div class="sicoob-config-block">
					<div class="sicoob-config-block-header">
						<h2>
							<span class="dashicons dashicons-admin-tools"></span>
							<?php esc_html_e( 'Teste da API', 'sicoob-payment' ); ?>
						</h2>
					</div>
					
					<div class="sicoob-config-block-content">
						<div class="sicoob-test-section">
							<h3><?php esc_html_e( 'Teste de Token de Acesso', 'sicoob-payment' ); ?></h3>
							<p><?php esc_html_e( 'Use este bloco para testar a comunicação com a API do Sicoob e verificar se as configurações estão corretas.', 'sicoob-payment' ); ?></p>
							
							<div class="sicoob-test-actions">
								<button type="button" id="test-pix-token" class="sicoob-btn sicoob-btn-secondary">
									<span class="dashicons dashicons-admin-network"></span>
									<?php esc_html_e( 'Testar Token PIX', 'sicoob-payment' ); ?>
								</button>
								
								<button type="button" id="test-boleto-token" class="sicoob-btn sicoob-btn-secondary">
									<span class="dashicons dashicons-admin-network"></span>
									<?php esc_html_e( 'Testar Token Boleto', 'sicoob-payment' ); ?>
								</button>
								
								<button type="button" id="test-pix-generation" class="sicoob-btn sicoob-btn-primary">
									<span class="dashicons dashicons-money-alt"></span>
									<?php esc_html_e( 'Testar Geração PIX', 'sicoob-payment' ); ?>
								</button>
								
								<button type="button" id="test-boleto-generation" class="sicoob-btn sicoob-btn-primary">
									<span class="dashicons dashicons-media-document"></span>
									<?php esc_html_e( 'Testar Geração Boleto', 'sicoob-payment' ); ?>
								</button>
							</div>
							
							<div id="api-test-results" class="sicoob-test-results" style="display: none;">
								<h4><?php esc_html_e( 'Resultado do Teste:', 'sicoob-payment' ); ?></h4>
				...
				<div class="sicoob-config-block">
					<div class="sicoob-config-block-header">
						<h2>
							...
							<?php esc_html_e( 'Webhook PIX', 'sicoob-payment' ); ?>
						</h2>
					</div>
					
					<div class="sicoob-config-block-content">
						<div class="sicoob-webhook-section">
							<h3><?php esc_html_e( 'Configuração do Webhook PIX', 'sicoob-payment' ); ?></h3>
							<p><?php esc_html_e( 'Configure o webhook para receber notificações automáticas de pagamentos PIX. O webhook deve aceitar apenas requisições POST na URL especificada.', 'sicoob-payment' ); ?></p>
							...
							<div class="sicoob-webhook-info">
								<div class="sicoob-webhook-url">
									<label><?php esc_html_e( 'URL do Webhook:', 'sicoob-payment' ); ?></label>
									<div class="sicoob-url-display">
										...
										<button type="button" class="sicoob-copy-url-btn" data-url="<?php echo esc_attr( $webhook_url ); ?>">
											...
											<?php esc_html_e( 'Copiar', 'sicoob-payment' ); ?>
										</button>
									</div>
								</div>
								...
								<?php if ( empty( $pix_key ) ) : ?>
									<div class="sicoob-webhook-warning">
										<span class="dashicons dashicons-warning"></span>
										<p><?php esc_html_e( 'Chave PIX não configurada. Configure a chave PIX nas configurações do gateway PIX para poder usar o webhook.', 'sicoob-payment' ); ?></p>
									</div>
								<?php endif; ?>
							</div>
							...
							<div id="webhook-test-results" class="sicoob-test-results" style="display: none;">
								<h4><?php esc_html_e( 'Resultado da Operação:', 'sicoob-payment' ); ?></h4>
								<div class="sicoob-test-response">
				...
				<div class="sicoob-config-block">
					<div class="sicoob-config-block-header">
						<h2>
							...
							<?php esc_html_e( 'Teste de E-mail do Boleto', 'sicoob-payment' ); ?>
						</h2>
					</div>
					
					<div class="sicoob-config-block-content">
						<div class="sicoob-test-section">
							<h3><?php esc_html_e( 'Teste de Envio de E-mail', 'sicoob-payment' ); ?></h3>
							<p><?php esc_html_e( 'Use este bloco para testar o envio de e-mail com dados do boleto. Um e-mail de teste será enviado com dados fictícios.', 'sicoob-payment' ); ?></p>
							
							<div class="sicoob-email-test-form">
								<div class="sicoob-config-field">
									<label for="test-email-address">
										<?php esc_html_e( 'E-mail de Teste', 'sicoob-payment' ); ?>
									</label>
									<input 
										type="email" 
										id="test-email-address" 
										placeholder="<?php esc_html_e( 'Digite seu e-mail para receber o teste', 'sicoob-payment' ); ?>"
										class="sicoob-email-input"
									>
									<div class="sicoob-config-field-description">
										<?php esc_html_e( 'Digite um e-mail válido onde você deseja receber o e-mail de teste do boleto.', 'sicoob-payment' ); ?>
									</div>
								</div>
								...
								<div class="sicoob-test-actions">
									<button type="button" id="test-boleto-email" class="sicoob-btn sicoob-btn-primary">
										<span class="dashicons dashicons-email-alt"></span>
										<?php esc_html_e( 'Enviar E-mail de Teste', 'sicoob-payment' ); ?>
									</button>
								</div>
							</div>
							
							<div id="email-test-results" class="sicoob-test-results" style="display: none;">
								<h4><?php esc_html_e( 'Resultado do Teste de E-mail:', 'sicoob-payment' ); ?></h4>
								<div class="sicoob-test-response">
	...
	public function ajax_check_payment_status() {
		// Verify nonce.
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) ), 'sicoob_pix_nonce' ) ) {
			wp_die( esc_html__( 'Nonce inválido.', 'sicoob-payment' ) );
		}
		...
		if ( ! $order_id ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'ID do pedido inválido.', 'sicoob-payment' ),
				)
			);
		}
		...
		if ( ! $order ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Pedido não encontrado.', 'sicoob-payment' ),
				)
			);
		}
		...
		if ( $is_paid ) {
			$response['message'] = esc_html__( 'Pagamento confirmado com sucesso!', 'sicoob-payment' );
		} else {
			$response['message'] = esc_html__( 'Aguardando confirmação do pagamento...', 'sicoob-payment' );
		}
