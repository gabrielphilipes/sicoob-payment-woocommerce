...
class WC_Sicoob_Boleto_Gateway extends WC_Payment_Gateway {
	...
	private function get_customer_name( $order ) {
		...
		if ( empty( $name ) ) {
			...
			if ( $customer_note ) {
				...
				$name = __( 'Cliente', 'sicoob-payment' );
			}
		}
	...
	public function display_boleto_payment_block( $order_id ) {
		...
		// Load template.
		$template_path = plugin_dir_path( __DIR__ ) . 'templates/boleto-payment-block.php';
	...
	private function display_boleto_payment_block_fallback( $order ) {
		...
		$formatted_due_date = $boleto_data_vencimento ? gmdate( 'd/m/Y', strtotime( $boleto_data_vencimento ) ) : '';
		?>
		<div class="sicoob-boleto-payment-block">
			<div class="sicoob-boleto-header">
				<h3><?php esc_html_e( 'Pagamento via Boleto', 'sicoob-payment' ); ?></h3>
				<p><?php esc_html_e( 'Visualize e imprima seu boleto bancário para realizar o pagamento', 'sicoob-payment' ); ?></p>
			</div>

			<div class="sicoob-boleto-content">
				<div class="sicoob-boleto-pdf-container">
					<div class="sicoob-boleto-pdf-header">
						<h4><?php esc_html_e( 'Boleto Bancário', 'sicoob-payment' ); ?></h4>
						<div class="sicoob-boleto-info">
							<span class="sicoob-boleto-value"><?php echo wp_kses_post( $formatted_value ); ?></span>
							<?php if ( $formatted_due_date ) : ?>
								<span class="sicoob-boleto-due-date">
									<?php
									// translators: %s is the due date.
									printf( esc_html__( 'Vence em: %s', 'sicoob-payment' ), esc_html( $formatted_due_date ) );
									?>
								</span>
							<?php endif; ?>
						</div>
					...
					<div class="sicoob-boleto-pdf-iframe">
						<iframe src="<?php echo esc_url( $boleto_pdf_url ); ?>" 
								...
								height="600" 
								frameborder="0"
								title="<?php esc_attr_e( 'Boleto Bancário', 'sicoob-payment' ); ?>"
				...
				<div class="sicoob-boleto-actions">
					<a href="<?php echo esc_url( $boleto_pdf_url ); ?>" 
						...
						target="_blank" 
						download>
						<?php esc_html_e( 'Baixar Boleto', 'sicoob-payment' ); ?>
					</a>
					
					<button type="button" class="sicoob-boleto-print-btn" id="sicoob-boleto-print-btn">
						<?php esc_html_e( 'Imprimir Boleto', 'sicoob-payment' ); ?>
					</button>
				</div>
			</div>
		</div>
