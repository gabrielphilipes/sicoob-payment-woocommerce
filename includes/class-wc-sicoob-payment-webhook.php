...
class WC_Sicoob_Payment_Webhook {
    ...
    private function process_pix_transaction($pix_transaction) {
        ...
        WC_Sicoob_Payment::log_message(
            sprintf(__('Processando transação PIX - TXID: %s, Valor: R$ %s, Horário: %s', 'sicoob-payment'), 
                $txid, 
        ...
        if (!$this->validate_payment_amount($order, $valor)) {
            WC_Sicoob_Payment::log_message(
                sprintf(__('Valor do pagamento não confere - Pedido: R$ %s, Pago: R$ %s', 'sicoob-payment'), 
    ...
    private function find_order_by_txid($txid) {
        ...
        if (!$order || $order->get_payment_method() !== 'sicoob_pix') {
            return false;
        }
    ...
    private function process_payment($order, $pix_transaction) {
        try {
            ...
            $order->add_order_note(
                sprintf(__('Pagamento PIX confirmado via webhook. TXID: %s, Valor: R$ %s', 'sicoob-payment'),
            ...
            WC_Sicoob_Payment::log_message(
                sprintf(__('Pagamento PIX processado com sucesso - Pedido #%s, TXID: %s', 'sicoob-payment'), 
                    $order->get_id(),
                    $pix_transaction['txid']
                ),
                'info'
            ...
            // Log error
            WC_Sicoob_Payment::log_message(
                sprintf(__('Erro ao processar pagamento PIX - Pedido #%s: %s', 'sicoob-payment'), 
                    $order->get_id(),
                    $e->getMessage()
                ),
                'error'
            ...
            $order->add_order_note(
                sprintf(__('Erro ao processar pagamento PIX via webhook: %s', 'sicoob-payment'), $e->getMessage())
            );
        }
    }
}
