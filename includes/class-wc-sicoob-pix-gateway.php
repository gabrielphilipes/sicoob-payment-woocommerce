...
/**
 * Sicoob PIX Payment Gateway Class
 *
 * @package SicoobPayment
 */
class WC_Sicoob_Pix_Gateway extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		$order->update_meta_data( '_sicoob_pix_txid', isset( $pix_data['txid'] ) ? $pix_data['txid'] : '' );
		$order->update_meta_data( '_sicoob_pix_qrcode', isset( $pix_data['brcode'] ) ? $pix_data['brcode'] : '' );
		$order->update_meta_data( '_sicoob_pix_criacao', isset( $pix_data['calendario']['criacao'] ) ? $pix_data['calendario']['criacao'] : '' );
		$order->update_meta_data( '_sicoob_pix_expiracao', isset( $pix_data['calendario']['expiracao'] ) ? $pix_data['calendario']['expiracao'] : 3600 );
		$order->save();
	...
	private function get_customer_name( $order ) {
		...
		if ( empty( $name ) ) {
			$name = $order->get_customer_note() ? $order->get_customer_note() : __( 'Cliente', 'sicoob-payment' );
		}
	...
	public function display_pix_payment_block( $order_id ) {
		...
		// Load template.
		$template_path = plugin_dir_path( __DIR__ ) . 'templates/pix-payment-block.php';
	...
	private function display_pix_payment_block_fallback( $order ) {
		...
		<div class="sicoob-pix-payment-block">
			<div class="sicoob-pix-header">
				<h3><?php esc_html_e( 'Pagamento via PIX', 'sicoob-payment' ); ?></h3>
				<p><?php esc_html_e( 'Escaneie o QR Code ou copie o código PIX para realizar o pagamento', 'sicoob-payment' ); ?></p>
			</div>
				...
				<div class="sicoob-pix-left">
					<div class="sicoob-pix-qr-container">
						...
						<p class="sicoob-pix-qr-text">
							<?php esc_html_e( 'Escaneie com o app do seu banco', 'sicoob-payment' ); ?>
						</p>
					</div>
				...
				<div class="sicoob-pix-right">
					<div class="sicoob-pix-instructions">
						<div class="sicoob-pix-step">
							...
							<h4 class="sicoob-pix-step-title"><?php esc_html_e( 'Abra o app do seu banco', 'sicoob-payment' ); ?></h4>
							<p class="sicoob-pix-step-text">
								<?php esc_html_e( 'Acesse o aplicativo do seu banco no celular ou internet banking.', 'sicoob-payment' ); ?>
							</p>
							</div>
						</div>
						...
						<div class="sicoob-pix-step">
							...
							<div class="sicoob-pix-step-content">
							<h4 class="sicoob-pix-step-title"><?php esc_html_e( 'Escolha a opção PIX', 'sicoob-payment' ); ?></h4>
							<p class="sicoob-pix-step-text">
								<?php esc_html_e( 'Procure pela opção "PIX" ou "Pagar com PIX" no menu principal.', 'sicoob-payment' ); ?>
							</p>
							</div>
						</div>
						...
						<div class="sicoob-pix-step">
							...
							<div class="sicoob-pix-step-content">
							<h4 class="sicoob-pix-step-title"><?php esc_html_e( 'Escaneie o QR Code', 'sicoob-payment' ); ?></h4>
							<p class="sicoob-pix-step-text">
								<?php esc_html_e( 'Use a câmera do app para escanear o QR Code ou cole o código PIX.', 'sicoob-payment' ); ?>
							</p>
							</div>
						</div>
						...
						<div class="sicoob-pix-step">
							...
							<div class="sicoob-pix-step-content">
							<h4 class="sicoob-pix-step-title"><?php esc_html_e( 'Confirme o pagamento', 'sicoob-payment' ); ?></h4>
							<p class="sicoob-pix-step-text">
								<?php esc_html_e( 'Verifique os dados e confirme o pagamento. O valor será debitado instantaneamente.', 'sicoob-payment' ); ?>
							</p>
							</div>
						</div>
						...
						<div class="sicoob-pix-step">
							...
							<div class="sicoob-pix-step-content">
							<h4 class="sicoob-pix-step-title"><?php esc_html_e( 'Aguarde a confirmação', 'sicoob-payment' ); ?></h4>
							<p class="sicoob-pix-step-text">
								<?php esc_html_e( 'O pedido será processado automaticamente após a confirmação do pagamento.', 'sicoob-payment' ); ?>
							</p>
							</div>
						</div>
					</div>
